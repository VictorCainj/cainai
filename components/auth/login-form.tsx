...
export function LoginForm({ onSwitchToRegister, onClose, onSuccess }: LoginFormProps) {
  ...
  const handleSocialLogin = async (provider: 'google' | 'github') => {
    try {
      ...
      if (result?.error) {
        setError(`Erro no login com ${provider === 'google' ? 'Google' : 'GitHub'}`)
      } else {
        // Login social bem-sucedido - o redirecionamento será feito pelo Supabase
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-400">Entre na sua conta para acessar suas conversas</p>
      </div>
